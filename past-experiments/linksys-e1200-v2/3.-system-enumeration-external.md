...
$ sudo lsof -i -P -n | grep -i listen  | grep :53
dnsmasq   23314    dnsmasq    5u  IPv4 179282      0t0  TCP *:53 (LISTEN)
dnsmasq   23314    dnsmasq    7u  IPv6 179284      0t0  TCP *:53 (LISTEN)
```

We may encounter the error: _dnsmasq: failed to create listening socket for port 53: Address already in_. In this case, we simply have to temporarily disable the process listening on this port (usually `systemd-resolved`):

```shellscript
$ sudo lsof -i -P -n | grep -i listen | grep :53
systemd-r   731 systemd-resolve   15u  IPv4  20237      0t0  TCP 127.0.0.53:53 (LISTEN)
systemd-r   731 systemd-resolve   17u  IPv4  20239      0t0  TCP 127.0.0.54:53 (LISTEN)

$ sudo vim /etc/systemd/resolved.conf
...
[Resolve]
DNS=8.8.8.8  # or any other preferred DNS server
DNSStubListener=no
...
